        ...
        if np.all(ids is not None):  # If there are markers found by detector
            ...
            for i in range(0, len(ids)):  # Iterate in markers
             # Estimate pose of each marker and return the values rvec and tvec---different from camera coefficients
                rvec, tvec, _= aruco.estimatePoseSingleMarkers(corners[i], 0.1, matrix_coefficients, distortion_coefficients)  # markerLength width 0.1m
                print('rvec', rvec)
                print('tvec', tvec)
                (rvec - tvec).any()  # get rid of that nasty numpy value array error
                aruco.drawDetectedMarkers(frame.copy(), corners, ids)  # Draw A square around the markers
                cv2.drawAxis(frame, matrix_coefficients, distortion_coefficients, rvec, tvec, 0.05)
